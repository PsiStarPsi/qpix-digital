...
randomFailRatePerASIC = 0.001
randomFailRatePerConn = 0.001
...
f2.write('progress report, cell i,j\n')
f2.close()
...
for i in range(0,nRows):
  ...
  for j in range(0,nCols):
    ...
    for c in range(0,CounterMC):
      ...
      comp_fail(asicMatrix)
      transmit = transmit_sim(asicMatrix,i,j)
      if(debugLevel>=1):
        print(transmit)
      success_transmit += float(transmit)
